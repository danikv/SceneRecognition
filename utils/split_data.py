...
loaded_dataset = dict(filter(lambda x : len(x[1]['labels']) != 0 and x[1]['num_frames'] < 100, loaded_dataset.items()))
print(len(loaded_dataset))
...
with open(os.path.join(args.output_folder, '1fps_100frames_train.pkl'), 'wb') as f:
    pickle.dump(train, f)

with open(os.path.join(args.output_folder, '1fps_100frames_test.pkl'), 'wb') as f:
    pickle.dump(test, f)
